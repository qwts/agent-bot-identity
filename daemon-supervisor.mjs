...
import { daemonStatus, stopDaemon } from './agent-daemon.mjs';
...
export function renderLaunchdPlist({ executable }) {
...
export function renderSystemdUnit({ executable }) {
  if (typeof executable !== 'string' || executable.length === 0 || executable.includes('\0')) {
    throw new Error('supervisor executable must be a non-empty path');
  }
...
export function renderSupervisorUnit({ kind, executable }) {
  if (kind === 'launchd') return renderLaunchdPlist({ executable });
  if (kind === 'systemd') return renderSystemdUnit({ executable });
...
function unloadSupervisor(paths, { env, exec, skipLoad }) {
  ...
  try {
    ...
    /* ignore reload failure after disable */
  }
  return { unloaded: true, skipped: false };
}
...
export async function ensureDaemonSupervisor({
  ...
  const body = renderSupervisorUnit({ kind: paths.kind, executable });
  ...
  const current = await probe({ env, home });
  if (current.running && !inspection.loaded && !skipLoad) {
    await stopDetached({ env, home });
  }
  const unchanged = previous === body && inspection.loaded;
  if (!unchanged) {
  ...
  const healthy = await waitHealthy({ env, home, probe });
  return {
    applied: true,
    loaded: true,
    skippedLoad: false,
    refreshed: previous !== body,
    alreadyRunning: Boolean(current.running && unchanged),
    pid: healthy.pid,
    port: healthy.port,
    platform,
    kind: paths.kind,
...
export async function disableDaemonSupervisor({
  ...
  const status = await probe({ env, home });
  if (status.running) await stop({ env, home });
  return {
    unloaded: true,
